...
public class Game {
	...
	public static final Cell[] cells = {
		...
		new Cell("成功廳",372,418,CellType.FATE).setScore(50).setMessage("猜兔子"),
		new Cell("光復操場",372,504,CellType.SELECT).setScore(-5).setMessage("晨練耍廢"),
		new Cell("第一活動中心",372,590,CellType.SELECT).setScore(30).setMessage("當上社團幹部").setSelectPolicy(SelectPolicy.ONLY_CLUB),
		new Cell("「飛撲」雕塑",372,676,CellType.SELECT).setScore(-20).setMessage("穿越飛撲，微積分被當").setSelectPolicy(SelectPolicy.ONLY_LESSON),
		new Cell("總圖書館",372,762,CellType.FATE).setScore(50).setMessage("知識王"),
		...
		new Cell("勝後",888,762,CellType.FATE).setScore(50).setMessage("接愛心"),
		...
		new Cell("成大醫院附設餐廳",1490,504,CellType.SELECT).setScore(15).setMessage("學生證優惠"),
